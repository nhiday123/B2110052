...
<body class="sub_page">
  ...
  <section>
    ...
    <?php
    // Thực hiện truy vấn SQL để lấy thông tin về các sản phẩm từ cơ sở dữ liệu
    $sql = "SELECT sanpham.*, chitietgiaodich.DonGia AS Gia_SP
            FROM sanpham
            INNER JOIN  chitietgiaodich ON sanpham.Ma_SP = chitietgiaodich.Ma_SP";
    $result = mysqli_query($conn, $sql);

    // Hiển thị thông tin chi tiết của sản phẩm đầu tiên
    if (mysqli_num_rows($result) > 0) {
        $first_row = mysqli_fetch_assoc($result);
    ?>
        <div class="left">
            <div class="main_image">
                <img src="<?php echo $first_row['HinhAnh_SP']; ?>" class="slide" width = "100%">
            </div>
        </div>

        <div class="right">
            <h3><?php echo $first_row['Ten_SP']; ?></h3>
            <h4> <small></small><?php echo number_format($first_row['Gia_SP']); ?> </h4>
...
if(isset($_GET['id'])) {
    ...
    $sql_products = "SELECT sanpham.*, chitietgiaodich.DonGia, danhmuc.Ten_DM
    FROM SANPHAM 
    JOIN chitietgiaodich ON sanpham.Ma_SP = chitietgiaodich.Ma_SP
            ...
            ?>
                    ...
                    <div class="name">
                        ...
                        <div class="detail-box">
                            ...
                            <h5>
                                <span><?php echo number_format($row['DonGia']); ?></span> <!-- Hiển thị giá của sản phẩm từ bảng đơn giá -->
                            </h5>
                            <a href="chitiet.php?id=<?php echo $row['Ma_SP']; ?>"> <!-- Điều hướng đến trang chi tiết sản phẩm với mã sản phẩm -->
                                Mua Ngay
                            </a>
                        </div>
                    </div>
